...
//            handler.addRepeatedDelayedMethod("doMoreBranches", secondLength, 5, (timesRun) -> doMoreBranches(horizontalDirection[secondWidthDirection]));
        ...
        public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
            if (!worldIn.isRemote) {
                ...
                GeomancyAndTerraforming.LOGGER.info("WORLD?" + worldIn);
            }
...
//                for(g = 0; g <= spellpower; ++g) {
                ...
                handler.addVariable("currenttargettest", targetblockpos);
                return "firstLoop";
        ...
        public String doFirstLoop() {
            World currentWorld = (World) handler.getVariable("currentWorld").get();
            BlockPos currenttargettest = (BlockPos) handler.getVariable("currenttargettest").get();

            GeomancyAndTerraforming.LOGGER.info("WTF IS THIS WORKING?");
            return "secondLoop";
        }

        private String doSecondLoop() {
            GeomancyAndTerraforming.LOGGER.info("WTF IS THIS WORKING? FOR REALZ>?!");
            return "initialize";
        }
    }
